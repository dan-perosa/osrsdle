...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch(`${BASE_URL}equipments/two_handed`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch(`${BASE_URL}daily_two_handed_equipment/`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const listAndBooleanVictory = await checkExistingVictory(token, 'two_handed')
    ...
    if (randomEquipment && randomEquipment.id === equipment.id) {
        ...
        if (jwtToken !== '') {
          ...
          await handleVictory(arrayToPassToApi, jwtToken, 'two_handed')
          return
        }
    }
    if (jwtToken !== '') {
      ...
      await updateUserSelectedList(jwtToken, 'two_handed', addToUserSelected)
    }
    // checks hints
