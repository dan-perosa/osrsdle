...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch(`${BASE_URL}equipments/neck`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch(`${BASE_URL}daily_neck_equipment/`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const listAndBooleanVictory = await checkExistingVictory(token, 'neck')
    ...
    if (randomEquipment && randomEquipment.id === equipment.id) {
        ...
        if (jwtToken !== '') {
          ...
          await handleVictory(arrayToPassToApi, jwtToken, 'neck')
          return
        }
    }
    if (jwtToken !== '') {
      ...
      await updateUserSelectedList(jwtToken, 'neck', addToUserSelected)
    }
    // checks hints
