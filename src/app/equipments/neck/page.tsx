...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch('http://127.0.0.1:5000/equipments/neck', {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch('http://127.0.0.1:5000/daily_neck_equipment/', {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const selectedList = await checkExistingVictory(token, 'neck')
