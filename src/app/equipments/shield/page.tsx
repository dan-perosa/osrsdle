...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch(`${BASE_URL}equipments/shield`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch(`${BASE_URL}daily_shield_equipment/`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const listAndBooleanVictory = await checkExistingVictory(token, 'shield')
    ...
    if (randomEquipment && randomEquipment.id === equipment.id) {
        ...
        if (jwtToken !== '') {
          ...
          await handleVictory(arrayToPassToApi, jwtToken, 'shield')
          return
        }
    }
    if (jwtToken !== '') {
      ...
      await updateUserSelectedList(jwtToken, 'shield', addToUserSelected)
    }
    // checks hints
