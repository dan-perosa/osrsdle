...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch(`${BASE_URL}equipments/cape`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch(`${BASE_URL}daily_cape_equipment/`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const listAndBooleanVictory = await checkExistingVictory(token, 'cape')
    ...
    if (randomEquipment && randomEquipment.id === equipment.id) {
        ...
        if (jwtToken !== '') {
          ...
          await handleVictory(arrayToPassToApi, jwtToken, 'cape')
          return
        }
    }
    if (jwtToken !== '') {
      ...
      await updateUserSelectedList(jwtToken, 'cape', addToUserSelected)
    }
    // checks hints
