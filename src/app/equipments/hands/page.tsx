...
const EquipmentsPage: React.FC = () => {
    ...
    const fetchEquipments = async () => {
      try {
      const response = await fetch(`${BASE_URL}equipments/hands`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      const response2 = await fetch(`${BASE_URL}daily_hands_equipment/`, {
            mode: 'cors',
            method: 'GET',
          });
      ...
      if (token) {
        const listAndBooleanVictory = await checkExistingVictory(token, 'hands')
    ...
    if (randomEquipment && randomEquipment.id === equipment.id) {
        ...
        if (jwtToken !== '') {
          ...
          await handleVictory(arrayToPassToApi, jwtToken, 'hands')
          return
        }
    }
    if (jwtToken !== '') {
      ...
      await updateUserSelectedList(jwtToken, 'hands', addToUserSelected)
    }
    // checks hints
